...
import { getAdminProducts, deleteProduct, updateProduct, type ProductWithRelations } from "@/services/products";
...
export default function AdminProducts() {
  ...
  const [availabilityProduct, setAvailabilityProduct] = useState<ProductWithRelations | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<ProductWithRelations | null>(null);
  const [deleting, setDeleting] = useState(false);

  const refresh = () => { setLoading(true); Promise.all([getAdminProducts(), getCategories()]).then(([p, c]) => { setProducts(p); setCategories(c); }).catch(() => toast.error("Failed to load products")).finally(() => setLoading(false)); };
  useEffect(() => { refresh(); }, []);
  const visibleProducts = useMemo(() => products.filter(p => {
    const matchesQuery = `${p.name} ${p.style ?? ""} ${p.categories?.name ?? ""}`.toLowerCase().includes(query.toLowerCase());
  ...
  return <>
    ...
    <div className="admin-list-panel">
      ...
      {loading ? <div className="admin-empty-state">Loading catalogue…</div> : visibleProducts.length === 0 ? <div className="admin-empty-state"><PackageOpen size={28} /><strong>{products.length ? "No matching products" : "Your catalogue is ready for its first piece"}</strong><span>{products.length ? "Try adjusting your search or filter." : "Add a product to start building your collection."}</span>{!products.length && <button className="admin-primary-button" onClick={() => setView("add")}><Plus size={15} /> Add first product</button>}</div> : <div className="admin-product-table"><div className="admin-product-table-head"><span>Product</span><span>Category</span><span>Rental price</span><span>Status</span><span /></div>{visibleProducts.map(product => <div className="admin-product-row" key={product.id}><div className="admin-product-main">{product.image ? <img src={product.image} alt="" loading="lazy" decoding="async" /> : <div className="admin-product-placeholder"><PackageOpen size={18} /></div>}<div><strong>{product.name}</strong><span>{product.style ?? "No style"} <i>·</i> {product.slug}</span></div></div><span className="admin-product-category">{product.categories?.name ?? "Uncategorised"}</span><span className="admin-price">{formatRentalPrice(product.rental_price)}<small> / rental</small></span><span className={`admin-status-pill ${product.availability.toLowerCase()}`}><i />{product.availability}</span><div className="admin-row-actions"><button onClick={() => { setEditingProduct(product); setView("edit"); }} title="Edit product"><Edit3 size={15} /></button><button onClick={() => { setAvailabilityProduct(product); setView("availability"); }} title="Manage availability"><Calendar size={15} /></button><button onClick={() => toggle(product, "is_public")} title={product.is_public ? "Hide product" : "Publish product"}>{product.is_public ? <Eye size={15} /> : <EyeOff size={15} />}</button><button className={product.is_featured ? "is-selected" : ""} onClick={() => toggle(product, "is_featured")} title="Toggle featured"><Star size={15} fill={product.is_featured ? "currentColor" : "none"} /></button><button className="is-danger" onClick={() => setDeleteTarget(product)} title="Delete product"><Trash2 size={15} /></button></div></div>)}</div>}
    </div>
